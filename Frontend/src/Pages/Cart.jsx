...
const Cart = () => {
    ...
    const {products,currency,cartItems,getCartAmount,removeFromCart,getCartCount,updateCartItems,navigate,axios,user,setCartItems}=useAppContext()
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button className="cursor-pointer mx-auto">
                            <img src={assets.remove_icon} className="inline-block w-6 h-6" alt="" onClick={()=>removeFromCart(product._id)}/>
                        </button>
                    </div>)
                )}

                <button onClick={()=>{navigate('/products'); scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 text-green-500 font-medium">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                ...
                                <p onClick={() => navigate('/addAddress')} className="text-green-500 text-center cursor-pointer p-2 hover:bg-indigo-500/10">
                                    Add address
                                </p>
                            </div>
                        )}
